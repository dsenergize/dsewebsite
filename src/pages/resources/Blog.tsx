...
// --- Configuration ---
const API_URL = 'https://strapi-dsewebsite-d05s.onrender.com/api/blog-posts?populate=*';
...
const Blog = () => {
    ...
    const fetchPosts = async () => {
      try {
        ...
        const normalizedApiPosts = validApiPosts.map(post => {
          const imageUrlPath = post.Image?.formats?.large?.url || post.Image?.url;
          const imageUrl = imageUrlPath
            ? `https://strapi-dsewebsite-d05s.onrender.com${imageUrlPath}`
          ...
          return {
            ...
            date: new Date(post.Published_on).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' }),
            imageUrl: imageUrl,
            ...
            <main className="lg:col-span-2">
              {currentPost ? (
                <article className="bg-gray-50 p-6 sm:p-8 rounded-2xl shadow-md">
                  <div className="mb-6">
                    <h2 className="text-3xl md:text-4xl font-bold text-blue-900 leading-tight">{currentPost.title}</h2>
                  ...
                  <div className="mb-8">
                    <img src={currentPost.imageUrl} alt={currentPost.altText} className="rounded-xl w-full h-auto object-cover shadow-lg" />
                  </div>
                  <div className="prose">
  <ReactMarkdown>{currentPost.content}</ReactMarkdown>
                 ...
                 !loading && <div className="text-center p-8 bg-gray-50 rounded-2xl shadow-md">No posts available.</div>
              )}
            </main>
            ...
            <aside className="space-y-8">
              <div className="bg-gray-50 p-6 rounded-2xl shadow-md">
                ...
                <p className="text-sm text-gray-600 mb-4">DS Energize is India's emerging innovator in smart solar monitoring and data-driven asset management.</p>
                     ...
                     {allPosts.map(post => (
                       <li key={post.slug}>
                         <button 
                            ...
                            className={`font-medium text-left w-full transition-colors duration-200 ${currentPost?.slug === post.slug ? 'text-blue-700' : 'text-gray-800 hover:text-blue-700'}`}
                         >
                           {post.title}
                         </button>
                       </li>
